...
export default function TransportRegister() {
  ...
  const [form, setForm] = useState({ employee_code:'', full_name:'', employee_type:'', plant_id:'', department_id:'', department_text:'', route_id:'', pickup_point:'' });
  ...
  const [successOpen, setSuccessOpen] = useState(false); // show success popup for non-admin save

  // precomputed burst particles for the success animation
  const bursts = useMemo(() => {
    const n = 10; const r = 64;
    const colors = ['#2ecc71', '#00c2c7', '#ffd166', '#f78fb3', '#a29bfe'];
  ...
  }, [successOpen, isAdminga]);

  const openAddModal = () => {
    setIsEdit(false);
  setForm({ employee_code:'', full_name:'', employee_type:'', plant_id:'', department_id:'', department_text:'', route_id:'', pickup_point:'' });
    setShowModal(true);
  };
  ...
  <div style={{ fontSize:24, fontWeight:900, color:'#2f3e4f', marginTop:4, marginBottom:8 }}>แก้ไขข้อมูลขึ้นทะเบียนรถรับส่ง</div>
      ...
      {/* Success Popup for non-admin users */}
      {successOpen && !isAdminga && (
        <div className="tr-success-overlay">
          <div className="tr-success-card">
            <div className="tr-icon-wrap">
              <svg viewBox="0 0 72 72" className="tr-check">
                <circle className="tr-circle" cx="36" cy="36" r="34" fill="none" stroke="#2ecc71" strokeWidth="4" />
                <path className="tr-tick" fill="none" stroke="#2ecc71" strokeWidth="5" strokeLinecap="round" strokeLinejoin="round" d="M20 37 L31 47 L52 26" />
              </svg>
              <div className="tr-burst">
                {bursts.map((b, i) => (
                  <span key={i} style={{ '--tx': b.tx, '--ty': b.ty, background: b.color }} />
                ))}
              </div>
            </div>
            <div className="tr-title">Success</div>
            <div className="tr-desc"></div>
            <button className="tr-btn" onClick={() => setSuccessOpen(false)}>ตกลง</button>
          </div>
          <style jsx>{`
            .tr-success-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.45); display: flex; align-items: center; justify-content: center; z-index: 2000; animation: tr-fade-in .2s ease-out; }
            .tr-success-card { width: min(90vw, 440px); background: #fff; border-radius: 16px; padding: 24px; text-align: center; box-shadow: 0 20px 60px rgba(0,0,0,0.20); animation: tr-pop .28s cubic-bezier(.18,.89,.32,1.28); }
            .tr-icon-wrap { width: 96px; height: 96px; margin: 0 auto; border-radius: 50%; background: radial-gradient(ellipse at center, #eafff4 0%, #e8f8ff 80%); position: relative; display: grid; place-items: center; box-shadow: 0 6px 14px rgba(46, 204, 113, 0.25) inset; }
            .tr-check { width: 72px; height: 72px; }
            .tr-circle { stroke-dasharray: 226; stroke-dashoffset: 226; animation: tr-draw 800ms ease-out forwards; }
            .tr-tick { stroke-dasharray: 70; stroke-dashoffset: 70; animation: tr-draw 620ms 280ms ease-out forwards; }
            .tr-title { font-weight: 900; font-size: 28px; color: #22313f; margin-top: 14px; }
            .tr-desc { color: #607d8b; margin-top: 6px; font-weight: 700; }
            .tr-btn { margin-top: 16px; background: #00c2c7; color: #fff; border: none; padding: 12px 18px; font-weight: 900; border-radius: 12px; cursor: pointer; }
            .tr-burst { position: absolute; inset: -8px; pointer-events: none; }
            .tr-burst span { position: absolute; width: 8px; height: 8px; border-radius: 50%; opacity: 0; animation: tr-burst 720ms 280ms ease-out forwards; }
            @keyframes tr-pop { 0% { transform: scale(.92); opacity: 0; } 100% { transform: scale(1); opacity: 1; } }
            @keyframes tr-fade-in { from { opacity: 0; } to { opacity: 1; } }
            @keyframes tr-draw { to { stroke-dashoffset: 0; } }
            @keyframes tr-burst { 0% { transform: translate(0,0) scale(.6); opacity: 0; } 40% { opacity: 1; } 100% { transform: translate(var(--tx), var(--ty)) scale(0); opacity: 0; } }
          `}</style>
        </div>
      )}
          ...
          <tbody>
            {sortedList.map((r) => (
              <tr key={r.id}>
                ...
                <td style={td}>{r.department_text || r.dept_code || ''}</td>
                <td style={td}>{r.route_name||''}</td>
                <td style={td}>{formatPickupDisplay(r.pickup_point)||''}</td>
                <td style={td}>
                  {isAdminga && (
                    <>
          ...
          <div style={{ background:'#fff', padding:20, borderRadius:16, width:'min(92vw, 680px)', minWidth:320, boxShadow:'0 4px 18px rgba(0,0,0,0.08)' }}>
            ...
            <form onSubmit={submitForm}>
              ...
              <div style={{ marginBottom:12 }}>
                ...
                <select value={form.plant_id} onChange={e=> setForm(prev=>({ ...prev, plant_id: e.target.value, department_id: '', department_text: '' }))} style={{ width:'100%', padding:'12px 14px', border:'1px solid #bdc3c7', borderRadius:14 }}>
              ...
              {/* ฝ่าย (พิมพ์เอง + แนะนำจากรายการฝ่ายของโรงงาน) */}
              <div style={{ marginBottom:12 }}>
                ...
                <div style={{ position:'relative' }}>
                  <input
                    ...
                    onChange={e=> setForm(prev=>({ ...prev, department_text: e.target.value, department_id: '' }))}
                    style={{ width:'100%', padding:'12px 14px', border:'1px solid #bdc3c7', borderRadius:14 }}
                  />
                  {(() => {
                    ...
                    return (
                      <div style={{ position:'absolute', zIndex:5, left:0, right:0, background:'#fff', border:'1px solid #dbe4ea', borderRadius:10, marginTop:6, boxShadow:'0 8px 20px rgba(0,0,0,0.08)' }}>
                        {sugg.map(d => (
                          <div key={d.id} onClick={()=> setForm(prev=>({ ...prev, department_text: d.code || d.name || '', department_id: d.id }))} style={{ padding:'8px 12px', cursor:'pointer' }}>
                            {(d.code || d.name)}
                          </div>
                        ))}
                      </div>
                    ...
                    {!form.route_id ? (
                      ...
                      <>
                        ...
                        {pickupPoints
                          ...
                          .map(pp => <option key={pp.id} value={pp.name}>{formatPickupDisplay(pp.name)}</option>)}
                      </>
                    )}
                  </select>
                  {isAdminga && (
                        ...
                        {pickupPoints
                          ...
                          .map(pp => (
                            <div key={pp.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'6px 8px' }}>
                              <div>{formatPickupDisplay(pp.name)}</div>
                            ...
                            <div style={{ flex:1 }}>
                              {isEditing ? (
                                <input value={editing.value} onChange={e=> setEditing(prev=>({ ...prev, value: e.target.value }))} style={{ width:'100%', padding:'8px 10px', border:'1px solid #bdc3c7', borderRadius:8 }} />
                                    ...
                                    <span>{formatPickupDisplay(pp.name)}</span>
                              )}
                            </div>
                            {isEditing ? (
                              <>
