...
export default function TruckTable() {
  ...
  // For display: move 19:50 to the far-left when night shift (no DB changes)
  const displayDepartTimes = useMemo(() => {
    const list = Array.isArray(departTimes) ? [...departTimes] : [];
    if (!isNightShift || list.length === 0) return list;
    const find1950Index = list.findIndex(dt => String(dt?.time||'').slice(0,5) === '19:50');
    if (find1950Index > 0) {
      const [item] = list.splice(find1950Index, 1);
      list.unshift(item);
    }
    return list;
  }, [departTimes, isNightShift]);
  //
  //
  //
  //
            ...
            <thead>
              <tr>
                ...
                {displayDepartTimes.map((dt, idx) => (
                  ...
                  >
                    {(() => { const t = String(dt.time).slice(0,5); const is1950 = t === '19:50'; const prefix = (isNightShift && is1950) ? 'เวลาเข้า' : 'เวลาออก'; return (<>{prefix} <strong>{t}</strong></>); })()}
                  </th>
                ))}
              </tr>
              <tr>
