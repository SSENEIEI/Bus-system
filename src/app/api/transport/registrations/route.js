...
export async function GET(request) {
  ...
  if (format === 'csv') {
    ...
    for (const r of rows) {
      ...
      const line = [r.employee_code, r.full_name, r.employee_type||'', r.plant_code||'', dept, r.route_name||'', r.pickup_point||'']
...
export async function POST(request) {
  ...
  await withInitRetry(() => query(
    `INSERT INTO transport_registrations (employee_code, full_name, employee_type, plant_id, department_id, department_text, route_id, pickup_point)
     VALUES (?,?,?,?,?,?,?,?)`,
    [employee_code, full_name, employee_type, plant_id, department_id, department_text, route_id, pickup_point]
  ));
...
export async function PUT(request) {
  ...
  await withInitRetry(() => query(
    `UPDATE transport_registrations SET employee_code=?, full_name=?, employee_type=?, plant_id=?, department_id=?, department_text=?, route_id=?, pickup_point=? WHERE id=?`,
    [employee_code, full_name, employee_type, plant_id, department_id, department_text, route_id, pickup_point, id]
  ));
  return NextResponse.json({ ok: true });
}
