-- Bus-system full schema and seed (optional)
-- Safe to run multiple times; uses IF NOT EXISTS and INSERT IGNORE where possible

-- 0) Database
CREATE DATABASE IF NOT EXISTS `Bus-system` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE `Bus-system`;

-- 1) Legacy tables (optional/back-compat)
...
-- Ensure new columns exist (MySQL 8.0.29+ supports IF NOT EXISTS)
...
ALTER TABLE users ADD COLUMN IF NOT EXISTS plant_id INT NULL;
ALTER TABLE users ADD COLUMN IF NOT EXISTS department_id INT NULL;

-- 3) Plants / Departments
...
ALTER TABLE departments ADD CONSTRAINT IF NOT EXISTS dept_plant_fk FOREIGN KEY (plant_id) REFERENCES plants(id) ON DELETE CASCADE;
ALTER TABLE users ADD CONSTRAINT IF NOT EXISTS users_plant_fk FOREIGN KEY (plant_id) REFERENCES plants(id) ON DELETE SET NULL;
ALTER TABLE users ADD CONSTRAINT IF NOT EXISTS users_department_fk FOREIGN KEY (department_id) REFERENCES departments(id) ON DELETE SET NULL;

-- 4) Shifts / Depart Times
...
ALTER TABLE depart_times ADD CONSTRAINT IF NOT EXISTS dt_shift_fk FOREIGN KEY (shift_id) REFERENCES shifts(id) ON DELETE CASCADE;
...
ALTER TABLE ot_counts ADD CONSTRAINT IF NOT EXISTS otc_route_fk FOREIGN KEY (route_id) REFERENCES routes(id) ON DELETE CASCADE;
ALTER TABLE ot_counts ADD CONSTRAINT IF NOT EXISTS otc_plant_fk FOREIGN KEY (plant_id) REFERENCES plants(id) ON DELETE CASCADE;
ALTER TABLE ot_counts ADD CONSTRAINT IF NOT EXISTS otc_dept_fk FOREIGN KEY (department_id) REFERENCES departments(id) ON DELETE CASCADE;
ALTER TABLE ot_counts ADD CONSTRAINT IF NOT EXISTS otc_shift_fk FOREIGN KEY (shift_id) REFERENCES shifts(id) ON DELETE CASCADE;
ALTER TABLE ot_counts ADD CONSTRAINT IF NOT EXISTS otc_dt_fk FOREIGN KEY (depart_time_id) REFERENCES depart_times(id) ON DELETE CASCADE;
...
ALTER TABLE ot_locks ADD CONSTRAINT IF NOT EXISTS otlock_user_fk FOREIGN KEY (locked_by_user_id) REFERENCES users(id) ON DELETE SET NULL;
...
ALTER TABLE bookings ADD CONSTRAINT IF NOT EXISTS bookings_user_fk FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE;
ALTER TABLE bookings ADD CONSTRAINT IF NOT EXISTS bookings_product_fk FOREIGN KEY (product_id) REFERENCES products(id) ON DELETE CASCADE;
